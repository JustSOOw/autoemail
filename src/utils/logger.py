        ...
        if hasattr(record, 'msg'):
            ...
            for pattern in self.SENSITIVE_PATTERNS:
                if pattern in msg.lower():
                    ...
                    msg = re.sub(pattern_regex, f'{pattern}=***', msg, flags=re.IGNORECASE)
...
class LogContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type is None:
            self.logger.info(f"完成执行: {self.operation} (耗时: {duration.total_seconds():.2f}秒)")
        else:
            self.logger.error(f"执行失败: {self.operation} (耗时: {duration.total_seconds():.2f}秒) - {exc_val}")
