...
class TestEmailModel(unittest.TestCase):
    ...
    def test_email_model_creation(self):
        """测试邮箱模型创建"""
        email = EmailModel(email_address="test@example.com", notes="测试邮箱")
    ...
    def test_email_model_serialization(self):
        """测试邮箱模型序列化"""
        email = EmailModel(
            email_address="test@example.com", tags=["测试用", "开发用"], notes="测试邮箱"
        )
...
class TestTagModel(unittest.TestCase):
    ...
    def test_tag_model_creation(self):
        """测试标签模型创建"""
        tag = TagModel(name="测试标签", color="#ff0000", icon="🏷️", description="这是一个测试标签")
...
class TestDatabaseService(unittest.TestCase):
    ...
    def test_database_operations(self):
        ...
        emails = self.db_service.execute_query(
            "SELECT * FROM emails WHERE email_address = ?", ("test@example.com",)
        )
...
class TestConfigManager(unittest.TestCase):
    ...
    def test_config_save_load(self):
        ...
        # 更新配置
        updates = {
            "domain_config": {"domain": "test.example.com"},
